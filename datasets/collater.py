...
class Collater(object):
    ...
    def __call__(self, batch):
        ...
        num_params1 = bboxes[0].shape[-1]
        max_num_boxes = max(bbox.shape[0] for bbox in bboxes)
        padded_boxes = torch.ones(batch_size, max_num_boxes, num_params1) * -1
        num_params2 = landmarks[0].shape[-1]
        padded_landmarks = torch.ones(batch_size, max_num_boxes, num_params2) * -1
        for i in range(batch_size):
            im, bbox, landmark = images[i], bboxes[i], landmarks[i]
            ...
            landmark[:, :2] = landmark[:, :2] * im_scale[:2]
            padded_boxes[i, :bbox.shape[0], :] = torch.from_numpy(bbox)
            padded_landmarks[i, :landmark.shape[0], :] = torch.from_numpy(landmark)
        return {'image': padded_ims, 'boxes': padded_boxes, 'landmarks': padded_landmarks}
